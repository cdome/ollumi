...
describe('MagicShelfComponent (Part 3)', () => {
  ...
  describe('buildRuleFromData edge cases', () => {
    it('should handle in_between for date fields', () => {
      const rule: Rule = {field: 'publishedDate', operator: 'in_between' as never, value: null, valueStart: '2024-01-01', valueEnd: '2024-12-31'};
      const formGroup = component.buildRuleFromData(rule);
    ...
    it('should handle in_between for number fields', () => {
      const rule: Rule = {field: 'pageCount', operator: 'in_between' as never, value: null, valueStart: 100, valueEnd: 500};
      const formGroup = component.buildRuleFromData(rule);
    ...
    it('should preserve array values for includes_any', () => {
      const rule: Rule = {field: 'readStatus', operator: 'includes_any' as never, value: ['READ', 'READING']};
      const formGroup = component.buildRuleFromData(rule);
      expect(formGroup.get('value')?.value).toEqual(['READ', 'READING']);
    });

    it('should handle is_empty operator with no value', () => {
      const rule: Rule = {field: 'title', operator: 'is_empty' as never, value: null};
      const formGroup = component.buildRuleFromData(rule);
    ...
    it('should handle string values for text fields', () => {
      const rule: Rule = {field: 'title', operator: 'contains' as never, value: 'Harry Potter'};
      const formGroup = component.buildRuleFromData(rule);
  ...
  describe('numericFieldConfigMap', () => {
    ...
    it('should not contain title', () => {
      const config = component.numericFieldConfigMap.get('title' as never);
  ...
  describe('buildGroupFromData', () => {
    it('should build group with mixed rules and nested groups', () => {
      ...
      expect((nestedGroup.get('rules') as FormArray).length).toBe(1);
    });
